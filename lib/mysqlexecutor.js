...
const logger = require("lvlog").getLogger("mysql");
...
class MysqlExecutor {
  ...
  async execute(sqlString) {
    logger.trace(`Trying to execute SQL string: [${sqlString}]`);
  ...
  async insert(table, obj) {
    ...
    const q = `INSERT INTO ${table}(${Object.keys(obj)}) VALUES(${vs})`;
  ...
  async update(table, newData, whereCond) {
    ...
    const q = `UPDATE ${table} SET ${s} WHERE ${constructWhere(whereCond)}`;
  ...
  async select(table, fields, whereCond, extra) {
    ...
    const w = whereCond ? `WHERE ${constructWhere(whereCond)}` : "";
    const q = `SELECT ${fields} FROM ${table} ${w} ${extra || ""}`;
